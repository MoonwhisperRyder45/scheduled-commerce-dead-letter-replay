...
> Design notes for **scheduled-commerce-dead-letter-replay** — a runnable go example that Replay failed checkout, fulfillment, receipt, and order-update jobs from a dead-letter queue on a cron schedule.
...
- **Feature code** — the actual task: ecommerce-dead-letter-replay.
